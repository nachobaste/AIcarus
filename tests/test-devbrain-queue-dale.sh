...
F=$(ls "$QUEUE_DIR"/*--first-proposal.plan.md 2>/dev/null | head -1)
...
grep -qi "approved" "$TMP/out1" || fail "no confirmation was printed for the assistant to relay"
...
# ---- no <n> "<reason>" discards and requires a reason -----------------------
cat >> "$BACKLOG" <<'EOF'
...
[ "$RC" -ne 0 ] || fail "'no' with no reason should be refused, not silently discarded"
grep -A6 "^### Third proposal$" "$BACKLOG" | grep -q "decision:" \
  && fail "'no' without a reason still marked the proposal decided"
echo "OK: 'no' without a reason is refused, the proposal stays pending"

run no 1 "does not apply to this cycle" > "$TMP/out4" 2>&1; RC=$?
[ "$RC" -eq 0 ] || fail "'no' with a reason should succeed, got $RC: $(cat "$TMP/out4")"
grep -q "decision: discarded.*does not apply to this cycle" "$BACKLOG" || fail "the reason was not recorded"
...
echo "OK: no <n> \"<reason>\" discards and records the reason, writes nothing to the queue"

# ---- malformed input never approves or discards anything -------------------
BEFORE=$(ls "$QUEUE_DIR"/*.plan.md 2>/dev/null | wc -l | tr -d ' ')
for BAD in "dale" "dale abc" "dale -1" "dale 0" "no" "no abc reason"; do
...
grep -qi "no pending" "$TMP/out5" || fail "the out-of-range error should be legible to relay back"
